...
class SSNP():
    ...
    def load_model(self, saved_model_folder):
        ...
        #
        self.dbm = Model(encoded_input, main_output.output)
    ...
    def fit(self, X, y=None, validation_split=0.05):
        ...
        dbm_input = Input(shape=(2,))
        l = model.get_layer('enc1')(dbm_input)
        l = model.get_layer('enc2')(l)
        l = model.get_layer('enc3')(l)
        classes = model.get_layer('main_output')(l)

        self.dbm = Model(inputs=dbm_input, outputs=classes)
    ...
    def transform(self, X):
        if self._is_fit():
            return self.fwd.predict(X, verbose=self.verbose)

    def inverse_transform(self, X_2d):
        if self._is_fit():
            return self.inv.predict(X_2d, verbose=self.verbose)

    def predict(self, X):
        if self._is_fit():
            y_pred = self.clustering.predict(X, verbose=self.verbose)
            return self.label_bin.inverse_transform(y_pred)

    def predict2d(self, X_2d):
        if self._is_fit():
            y_pred = self.dbm.predict(X_2d, verbose=self.verbose)
            return self.label_bin.inverse_transform(y_pred)
